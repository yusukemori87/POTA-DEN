...
index_html = wrap(index_html, f'<link rel="canonical" href="{SITE}/"><script>window.POTADEN_PAGE_BASE="p/";</script>')
...
def product_page(i, p):
    ...
    t = p.get('tenno') or {}
    rv = p.get('review') or {}
    title = f"{p['brand']} {p['model']} のレビュー・評価と最安値｜ポタ電カタログ"
    desc = (f"{p['brand']} {p['model']}（{fmt(p.get('capacity_wh'))}Wh／定格{fmt(p.get('rated_output_w'))}W）の"
    ...
    ld = {"@context": "https://schema.org", "@type": "Product", "name": f"{p['brand']} {p['model']}",
    ...
    crumbs = {"@context": "https://schema.org", "@type": "BreadcrumbList", "itemListElement": [
        ...
        {"@type": "ListItem", "position": 2, "name": f"{p['brand']} {p['model']}", "item": url}]}
    sc = t.get('score') or 0
...
<main>
<nav style="font-size:12px;color:var(--muted);margin-bottom:10px"><a href="{SITE}/">ポタ電カタログ</a> ＞ {e(p['brand'])} {e(p['model'])}</nav>
<article class="modal" style="position:static;box-shadow:none;border:1px solid var(--line)">
<div class="mimg">{f'<img src="{img}" alt="{e(p["brand"])} {e(p["model"])} の製品画像" width="280" height="280">' if img else f'<div class="noimg"><b>{fmt(p.get("capacity_wh"))}</b>Wh</div>'}</div>
<div class="mbody">
<div class="brand">{e(p['brand'])}</div><h1 style="margin:0;font-size:24px;line-height:1.25">{e(p['model'])}</h1>
